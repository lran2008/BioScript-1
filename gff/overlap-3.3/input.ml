  ...
  let n = Array.length tab in
    ...
    if (n==8) then
      ...
      begin
        ...
        begin
          (* // This gives a warning: Warning P: this pattern-matching is not exhaustive.
             // But the error is caught anyway *)
...
let fill_feat_arr_hash h infile =
  let inchan = open_in infile and stop = ref false and hintref = Hashtbl.create (Hashtbl.length h) in
    while (not !stop) do
    (
      try
...
let make_feat_arr_hash maxseq infile = 
  ...
  let _u = Common.print_log "# Names of sequences retrieved\n" in
    
  (* Create the actual hashtable of feature arrays *)
  let h = Hashtbl.create (Hashtbl.length hseq) in
  let _u = Common.print_log "# Hashtable of feature arrays created\n" in
    ...
    ignore (fill_feat_arr_hash h infile);
