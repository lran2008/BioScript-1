...
my $snpeff_path = "$root/tool/3.4i/snpEff";
...
if ($genome_version eq "GRCh37.71"){
	$snpeff_option = "-c $snpeff_config -motif -nextprot -v $genome_version -s $filepath$filename";
}else{
	$snpeff_option = "-c $snpeff_config -v $genome_version -s $filepath$filename";
}	
